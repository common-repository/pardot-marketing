...
class PardotMarketing_Prospects_Table extends WP_List_Table {
  function __construct() {
    global $status, $page;

    $args = [
      'singular'  => __( 'Pardot Prospect', 'pardotmarketing' ),
      'plural'    => __( 'Pardot Prospects', 'pardotmarketing' ),
      'ajax'      => false
    ];
  ...
  function get_columns() {
    ...
    $columns = [
      ...
      'company'     => __( 'Company', 'pardotmarketing' ),
  ...
  function get_sortable_columns() {
    $sortable_columns = [
      ...
      'last_activity_at' => [ 'last_activity_at', false ],
    ];
  ...
  function column_default( $item, $column_name ) {
    switch( $column_name ) {
      case 'id':
        return '<a href="https://pi.pardot.com/prospect/read/id/' .  $item['id'] . '" target="_blank" rel="noopener noreferrer">' . $item['id'] . '</a>';
      break;
      case 'company':
  ...
  function extra_tablenav( $which ) {
    ...
    <div class="alignleft actions">
      ...
      $current_assigned       = ! empty( $_REQUEST['assigned'] ) ? sanitize_text_field( $_REQUEST['assigned'] ) : false;
      ...
      ?>
      <select name="assigned" id="filter-by-assigned">
        <option value=""><?php _e( 'Assigned &amp; unassigned', 'wpzerospam' ); ?></option>
        <option<?php if ( $current_assigned == 'assigned' ): ?> selected="selected" <?php endif; ?> value="assigned"><?php _e( 'Assigned', 'wpzerospam' ); ?></option>
        <option<?php if ( $current_assigned == 'unassigned' ): ?> selected="selected" <?php endif; ?> value="unassigned"><?php _e( 'Unassigned', 'wpzerospam' ); ?></option>
      </select>
  ...
  function prepare_items($args = []) {
    ...
    $assigned = ! empty( $_REQUEST['assigned'] ) ? sanitize_text_field( $_REQUEST['assigned'] ) : false;
    if ( 'assigned' == $assigned  ) {
      $query_args['assigned'] = true;
    } elseif( 'unassigned' == $assigned ) {
      $query_args['assigned'] = false;
    }
    ...
    $data = pardotmarketing_get_prospects( $query_args );
